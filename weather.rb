...
def iterate(weather)
  weather.each do |key, value|
    puts "The city is #{location(weather)}. The description is #{description(weather)}. The temperature is #{weather[:temperature_farenheit]}."
  end
end
...
def convert_temp(weather)
  weather[:temperature_farenheit] = ((weather[:temperature_farenheit] - 32)*(5/9)).round(2)
